-- =====================================================
-- RT RW App - Fixed Supabase PostgreSQL Schema
-- Safe for fresh Supabase public schema / reset dummy data
-- =====================================================

-- 0. Prepare private schema for internal RLS helper functions
CREATE SCHEMA IF NOT EXISTS private;

-- 1. Clean existing objects safely
-- Drop helper functions first. CASCADE removes dependent old policies/triggers if they exist.
...
-- Fixed search_path to avoid Supabase Advisor warning: function_search_path_mutable
...
-- iuran_types MUST be created before iuran_periods and iuran_payments.
...
CREATE TABLE public.iuran_periods (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    housing_id UUID REFERENCES public.housing_profiles(id) ON DELETE CASCADE,
    iuran_type_id UUID REFERENCES public.iuran_types(id) ON DELETE SET NULL,
    title TEXT,
    month TEXT NOT NULL,
...
-- 8. RLS helper function in private schema, not exposed as public RPC
...
-- Reports
CREATE POLICY "User read own reports" ON public.reports FOR SELECT USING (
  created_by = auth.uid()
  OR resident_id IN (SELECT id FROM public.residents WHERE profile_id = auth.uid())
);
CREATE POLICY "User insert own reports" ON public.reports FOR INSERT WITH CHECK (
  created_by = auth.uid()
  OR resident_id IN (SELECT id FROM public.residents WHERE profile_id = auth.uid())
);
CREATE POLICY "Admin manage reports" ON public.reports FOR ALL USING (private.is_admin()) WITH CHECK (private.is_admin());
...
-- Iuran Types. IDs are valid UUIDs; no t/p/w-prefixed invalid UUIDs.
